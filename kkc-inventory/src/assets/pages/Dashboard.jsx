...
function weekLabel(isoYearWeek) {
  ...
  mon.setUTCDate(base.getUTCDate() + (dow === 0 ? -6 : 1 - dow));
  ...
  firstMon.setUTCDate(first.getUTCDate() + (fdow === 0 ? -6 : 1 - fdow));
  const weekInMonth = Math.floor((mon - firstMon) / 86400000 / 7) + 1;
  return `Week ${weekInMonth} of ${MONTHS[month]}`;
}

function withWarehouseParam(base, wid) {
  if (!wid) return base;
  const sep = base.includes('?') ? '&' : '?';
  return `${base}${sep}warehouse_id=${encodeURIComponent(wid)}`;
}

function KpiCard({ title, icon: Icon, value, iconColor = 'text.primary', onClick }) {
  return (
    <Card
      sx={{
        height: '100%',
...
function RowBill({ name, provider, due, amount }) {
  ...
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, py: 1 }}>
      ...
      <Chip
        size="small"
        label={`Due ${new Date(due).toLocaleDateString()}`}
        color={color}
        variant="outlined"
      />
...
export default function Dashboard() {
  ...
  const loadAll = React.useCallback(async () => {
    setLoading(true);
    try {
      const [sum, ser, od, top] = await Promise.all([
        fetchJSON(withWarehouseParam(`${API}/dashboard/summary`, selectedWarehouseId)),
        fetchJSON(
          withWarehouseParam(`${API}/dashboard/series?mode=${mode}`, selectedWarehouseId)
        ),
        fetchJSON(
          withWarehouseParam(
            `${API}/dashboard/outstanding-deliveries?limit=10`,
            selectedWarehouseId
          )
        ),
        fetchJSON(
  ...
  const chartTitle =
    mode === 'daily'
      ? "This Day's Sales and Purchases"
      : mode === 'weekly'
        ? "This Week's Sales and Purchases"
        : "This Month's Sales and Purchases";

  const xTick = (v) =>
    mode === 'daily' ? fmtDaily(v) : mode === 'weekly' ? weekLabel(v) : fmtMonthly(v);
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
        ...
        <Card sx={{ borderRadius: 2, boxShadow: 3 }}>
          <CardContent>
            ...
            {topProducts && topProducts.length > 0 ? (
              ...
              <Typography variant="body2" color="text.secondary">
                No sales data yet for this warehouse.
              </Typography>
            )}
          </CardContent>
        </Card>
      ...
      {/* Upcoming bills (static pa) */}
      <Card sx={{ borderRadius: 2, boxShadow: 3 }}>
        <CardContent>
          <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 1 }}>
            <EventNoteIcon />
            <Typography variant="h6" fontWeight={700}>
              Upcoming Due Dates (Bills)
            </Typography>
          </Stack>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            Static Data
